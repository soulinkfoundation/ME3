...
export const ASSISTANT_JOB_STARTER_RECIPES = [
  {
    id: "weekly-review",
    name: "Weekly Review",
    outcome: "Gather unfinished tasks, completed work, pending approvals, and project changes.",
    ...
    defaultDraft: draft({
      name: "Weekly Review",
      purpose: "Prepare a weekly Mission Control review packet.",
  ...
  {
    id: "task-carry-over",
    name: "Task Carry-Over",
    outcome: "Collect unfinished tasks and prepare tomorrow or next-week carry-over choices.",
    firstVersion: "core_v1",
    state: "ready",
    requiredCapabilityIds: ["mission.task.read", "mission.review_packet.create", "mission.task.create"],
    optionalCapabilityIds: [],
    recommendedSkillIds: [],
    requiredSkillIds: [],
    defaultDraft: draft({
      name: "Task Carry-Over",
      purpose: "Prepare carry-over choices for unfinished tasks.",
      recipeId: "task-carry-over",
      trigger: dailySchedule,
      destination: missionDestination("review_packet"),
      actions: [
        action("read-tasks", "mission.task.read", "Read unfinished tasks", "none"),
        action(
          "create-review-packet",
          "mission.review_packet.create",
          "Create carry-over review packet",
          "review_required",
        ),
  ...
  },
  {
    ...
    optionalCapabilityIds: ["daemon.metadata.read", "daemon.file.read"],
    recommendedSkillIds: [],
    requiredSkillIds: [],
    defaultDraft: draft({
      name: "Project Digest",
      purpose: "Summarize recent Mission Control project activity.",
      recipeId: "project-digest",
      trigger: weeklySchedule,
      destination: missionDestination("review_packet"),
      actions: [
        action("read-project", "mission.project.read", "Read project state", "none"),
        action("read-tasks", "mission.task.read", "Read project tasks", "none"),
        action(
          "create-review-packet",
          "mission.review_packet.create",
          "Create project digest packet",
          "review_required",
        ),
      ],
    }),
  },
  {
    ...
    firstVersion: "core_v1",
    state: "ready",
    requiredCapabilityIds: ["mission.approval.read", "mission.review_packet.create"],
    optionalCapabilityIds: [],
    recommendedSkillIds: [],
    requiredSkillIds: [],
    defaultDraft: draft({
      name: "Approval Sweep",
      purpose: "Show pending approvals that still need a decision.",
      recipeId: "approval-sweep",
      trigger: dailySchedule,
      destination: missionDestination("review_packet"),
      actions: [
        action("read-approvals", "mission.approval.read", "Read pending approvals", "none"),
        action(
          "create-review-packet",
          "mission.review_packet.create",
          "Create approvals review packet",
          "review_required",
        ),
      ],
    }),
  },
  {
    id: "memory-review",
    name: "Memory Review",
    outcome: "Gather useful memory suggestions without silently making them durable.",
    firstVersion: "core_v1",
    state: "ready",
    requiredCapabilityIds: ["mission.project.read", "mission.task.read", "mission.memory.suggest"],
    optionalCapabilityIds: ["mission.review_packet.create"],
    recommendedSkillIds: [],
    requiredSkillIds: [],
    defaultDraft: draft({
      name: "Memory Review",
      purpose: "Suggest useful private memory for owner review.",
      recipeId: "memory-review",
      trigger: weeklySchedule,
      destination: missionDestination("memory_review"),
      actions: [
        action("read-projects", "mission.project.read", "Read project context", "none"),
        action("read-tasks", "mission.task.read", "Read recent tasks", "none"),
        action(
          "suggest-memory",
  ...
  },
  {
    ...
    defaultDraft: draft({
      name: "Setup Health Check",
      purpose: "Surface setup issues for Assistant Jobs.",
      recipeId: "setup-health-check",
      trigger: dailySchedule,
      destination: missionDestination("activity"),
      actions: [
        action("validate-jobs", "assistant.job.validate", "Validate assistant jobs", "none"),
        action("create-activity", "mission.activity.create", "Create setup activity", "none"),
      ],
    }),
  },
  {
  ...
  },
  {
    ...
    firstVersion: "later_provider_adapter",
    state: "needs_setup",
    requiredCapabilityIds: ["email.message.read", "calendar.event.read", "mission.review_packet.create"],
    optionalCapabilityIds: ["mission.task.create"],
    recommendedSkillIds: [],
    requiredSkillIds: [],
    defaultDraft: draft({
      name: "Relationship Follow-Up",
      purpose: "Prepare relationship follow-up suggestions.",
      recipeId: "relationship-follow-up",
      trigger: weeklySchedule,
      destination: missionDestination("review_packet"),
      actions: [
        action("read-email", "email.message.read", "Read scoped email", "none"),
        action("read-calendar", "calendar.event.read", "Read scoped calendar", "none"),
        action(
          "create-review-packet",
          "mission.review_packet.create",
          "Create relationship follow-up packet",
          "review_required",
        ),
      ],
    }),

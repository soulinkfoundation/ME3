...
- The notification should stay lightweight and conversational:
  `📊 Your weekly review is ready in Mission Control 🚀. You have X open tasks, and Y completed over the last 7 days.`
...
8. The modal detects `metadata.kind === "weekly_review"` and renders the review UI:
   ...
   - Strict memory suggestions only when the source qualifies.
   - A custom memory textarea for the owner to add one important weekly data point.
...
- How should the Journal plugin expose the last seven days of notes to Assistant Jobs when it is enabled?
- Should strict memory suggestions be generated by an LLM over journal-derived candidate facts, or should the LLM only score/filter candidates produced by deterministic extractors?

## Future Journal And LLM Memory Slice

Automatic memory suggestions should not be inferred from open task titles. A candidate should only qualify when it is a durable fact, decision, preference, relationship/context detail, recurring constraint, or meaningful pattern from the previous seven days.

Recommended future flow:

1. If the Journal plugin is enabled, request the last seven days of journal notes through a plugin-owned API or context provider.
2. Generate candidate facts from those notes with a strict prompt/rubric:
   - reject ordinary tasks, reminders, one-off todos, vague reflections, and low-confidence guesses;
   - prefer explicit decisions, commitments, preferences, important dates, recurring constraints, named entities, and stable owner context;
   - include a confidence score and a short reason.
3. Compare candidates against existing Mission Control memory for duplicates.
4. Only surface high-confidence, non-duplicate candidates in the Weekly Review task.
5. Keep the custom memory textarea even when suggestions exist, because owner-authored weekly memory is likely to be more valuable than model guesses.

...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Core chat actions         | Partial parity. Reminder create/list, booking lookup, and mailbox draft save-from-chat follow-up have started.                                                                                                                                                                              | Hosted actions could be ported unevenly without shared capability/audit policy, and mailbox drafting still needs richer compose/reply cards.                                                     | `me3-tlt`                                            |
...
| Context and memory        | Native context packet contract, resolvers, manifests, chat wiring, memory review, job-run wiring, and high-priority Mission Control mission statement / Wheel of Life chat context exist.                                                                                                  | Scheduled jobs and model-backed job outputs still need richer context use.                                                                                                                      | `me3-ctx.8`                                          |
...
| Scheduler and reliability | Assistant Job event ingress uses Cloudflare Queues and a DLQ. Schedule editing and due scheduled-job dispatch through cron/queue now exist. Heartbeat/reconciliation design exists.                                                                                                         | Manual runs are synchronous, event trigger UI/filtering and heartbeat/reconciliation are still incomplete, and plugin queues such as booking reminders/social publishing are not wired in Core. | `me3-wsn.11`, `me3-wsn.22`, `me3-tlt.1`, `me3-1dr.1` |
...
- Continue mailbox draft actions beyond the first save-from-chat follow-up path: explicit compose, reply-to-thread, edit, and approval card behavior.
...
- Implement scheduled/manual/event trigger behavior. Current `me3-wsn.11` status: schedule editing plus due cron-to-queue dispatch exist; remaining work is event trigger UI/filtering, heartbeat/reconciliation, and durable scheduling beyond the minute cron.
...
- Keep the Mission Control mission statement and latest Wheel of Life snapshot as high-priority,
  lightweight context. Highlight either source as missing in owner-facing settings when it is not
  ready.
...
| Invoice and Receipt Triage | Extract receipts and invoices from email and add them to an accounts ledger.   | Satisfactory for now: run writes Mission Control activity and can create Accounts entries for likely invoices/receipts.   |
